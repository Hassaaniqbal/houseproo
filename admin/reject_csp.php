...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (isset($_POST["providerId"])) {
    ...
    $sql = "UPDATE company_service_provider SET status = 'rejected' WHERE provider_id = ?";
    $stmt = $mysqli->prepare($sql);
    $stmt->bind_param("s", $providerId);

    if ($stmt->execute()) {
      // Fetch provider's email for sending rejection notification
      $emailQuery = "SELECT email, firstname, lastname FROM company_service_provider WHERE provider_id = ?";
      ...
      $emailMessage .= "<b>After careful review and consideration, we regret to inform you that we have chosen not to move forward with your application at this time. We appreciate the time and effort you dedicated to the application process and wish you the best in your future endeavors.<br>";
      $emailMessage .= "Thank You<br><br>";
      $emailMessage .= "Best Regards,<br>";
      $emailMessage .= "House Pro Team";

      // Send the email
      Mail::sendMail($providerEmail, $providerFirstName . ' ' . $providerLastName, "Rejection Email", $emailMessage);//change the exact mail
