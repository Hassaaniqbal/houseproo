...
$averageRating = 0; // Initialize average rating variable
...
if (isset($_POST['providerId']) && is_numeric($_POST['providerId'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        echo '<div style="display: flex; align-items: center;">
            <img src="../service provider/uploads/pictures/' . $row['picture'] . '" class="img-fluid rounded" style="height: 12rem; width: 12rem;" alt="Profile Picture" />
            <div>
        ...
        echo "<p><strong>Company Working For:</strong> " . $row['companyname'] . "</p>";
        echo "<p><strong>Date Joined:</strong> " . $row['date'] . "</p>";
        echo '<hr>';

        // Step 1: Query bookings table to get relevant booking_ids
        $bookingSql = "SELECT booking_id FROM bookingg WHERE provider_id = ? AND sptype = 'company' AND status = 'completed' AND review_status = 'completed'";
