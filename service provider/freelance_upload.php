...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        // Update verification status in another table
            ...
            $query = "UPDATE freelance_service_provider SET verification_completed = ? WHERE provider_id = ?";
            ...
            move_uploaded_file($picture["tmp_name"], "uploads/freelance sp/pictures/" .  $picName);
            
            // Handle other file uploads (NIC, Certificate)
            ...
            foreach ($nic["name"] as $key => $filename) {
                ...
                $nicPath = $nicName;
                if (move_uploaded_file($nic["tmp_name"][$key], "uploads/freelance sp/nics/" .  $nicPath)) {
            ...
            foreach ($certificate["name"] as $key => $filename) {
                ...
                if (move_uploaded_file($certificate["tmp_name"][$key], "uploads/freelance sp/certificates/" . $certificatePath)) {
            ...
            $success = saveToDatabase($provider_id, $picName,  $finalNicData, $finalCertData, $address, $zipcode, $currentDate);
...
function saveToDatabase($provider_id, $picture, $nic, $certificate, $address, $zipcode, $currentDate) {
    global $mysqli; // Assuming $mysqli is your database connection object
    
    $query = "INSERT INTO freelance_sp_documents (provider_id, picture, NIC, certificate, address, zipcode, date) VALUES (?, ?, ?, ?, ?, ?, ?)";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param("sssssss", $provider_id, $picture, $nic, $certificate, $address, $zipcode, $currentDate);
    $result = $stmt->execute();
    $stmt->close();
    return $result;
}
